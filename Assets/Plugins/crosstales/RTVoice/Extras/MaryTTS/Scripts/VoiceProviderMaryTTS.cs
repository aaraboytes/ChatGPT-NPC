...
namespace Crosstales.RTVoice.MaryTTS
{
   ...
   public class VoiceProviderMaryTTS : Crosstales.RTVoice.Provider.BaseCustomVoiceProvider
   {
      ...
      [Tooltip("Configure the provider for a local server on the same device/network (default: false)."), SerializeField]
      ...
      /// <summary>Configure the provider for a local server on the same device/network (default: false).</summary>
      public bool isLocal
      {
         get => _isLocal;
         set => _isLocal = value;
      ...
      public override bool isOnlineService => !isLocal;
      ...
      public override void Load(bool forceReload = false)
      {
         ...
         if ((!isOnlineService || Crosstales.Common.Util.NetworkHelper.isInternetAvailable) && (cachedVoices?.Count == 0 || _forceReload))
         {
